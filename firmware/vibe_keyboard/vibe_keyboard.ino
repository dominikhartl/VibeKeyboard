...
// Per-LED brightness trim (1.0 = unchanged). Bump a dim/under-driven LED or balance
// colors that look uneven. Values >1 brighten; the result is clamped at full.
...
const bool          WAVE_BOUNCE    = true;   // true = sweep back and forth; false = one-way loop
...
struct Button {
  ...
  float         gain;         // per-LED brightness trim
};

// Listed LEFT -> RIGHT so the wave flows across the physical layout in this order.
Button buttons[] = {
  { PIN_VOICE,  PIN_VOICE_LED,  VOICE_KEY,  true,  false, false, false, 0, LED_GAIN_VOICE  }, // left
  { PIN_ACCEPT, PIN_ACCEPT_LED, ACCEPT_KEY, false, false, false, false, 0, LED_GAIN_ACCEPT }, // mid
  { PIN_REJECT, PIN_REJECT_LED, REJECT_KEY, false, false, false, false, 0, LED_GAIN_REJECT }, // right
};
...
void writeLed(uint8_t pin, uint8_t level, float gain = 1.0f) {
  ...
  if (v > 255) v = 255;                       // clamp at full brightness
